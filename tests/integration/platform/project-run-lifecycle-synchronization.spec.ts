...
describe('Project Git synchronization with the Run lifecycle adapter', () => {
  it('disposes all rebound workspaces under one project gate before committing', async () => {
    ...
    lifecycle.track(mainOwner, async () => {
      disposedWorkspaces.push(mainOwner.workspaceName)
    })
    lifecycle.track(worktreeOwner, async () => {
      disposedWorkspaces.push(worktreeOwner.workspaceName)
    })
    ...
    expect(disposedWorkspaces).toEqual(['main', 'feature/sidebar'])
  ...
  it('quarantines every rebound workspace when the synchronized snapshot cannot be saved', async () => {
    const repository = new InMemoryProjectRepository()
...
function createOwner(
  ...
  return {
    ...
    workspaceDirectory,
    gitBranch,
    blockId: `terminal-${workspaceName}`
  }
}
