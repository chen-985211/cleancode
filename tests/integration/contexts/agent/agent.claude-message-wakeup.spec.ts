...
describe('Claude native inbox wakeup', () => {
  ...
  it('acknowledges only the current notification through an authenticated native hook', async () => {
    const artifacts = new AgentLaunchArtifactScope()
    ...
    const plan = await provider.launcher.createLaunchPlan({
      artifacts,
      workspaceDirectory: process.cwd(),
      providerVersion: '2.1.261 (Claude Code)',
    ...
    try {
      ...
      const hook = settings.hooks.FileChanged.find((group: { matcher?: string }) => !group.matcher)
        .hooks[0]
