...
视觉整理以完整对象为排版单元。组合连同成员、流程连同全部终端一起移动；网格和全画布整理可以按既有依赖布局规则收紧流程内部间距，其他动作保持内部相对布局。所有整理都保持依赖连接、组合边界语义和节点尺寸。
...
- **整理画布**：从画布空白菜单作用于当前工作区全部完整对象，包括视野外的对象。Agent 构成左侧区域，独立终端、完整流程和完整组合构成右侧区域；两区分别复用紧凑网格，区域间保留 `96` 画布单位间距，各区垂直居中，整体外框中心固定为画布坐标原点。同尺寸对象用稳定身份决定顺序，不依赖之前的位置或输入顺序；重复执行不改变最终布局。只有一类对象时只排列该区域，单对象直接居中。完整流程复用网格的依赖层级收紧规则，组合不拆分，全部已有有效吸附关系在位置提交成功后解除。该动作复用网格动效；位置提交成功后，Presentation 根据最终布局外框而非尚未更新的节点投影，适配实际安全视口并整体居中。最大缩放为 `100%`，最小缩放沿用画布边界；超过最小缩放可容纳范围时保留整体居中。空画布和整理进行中不可再次执行。
...
网格和全画布整理的提交、关系解除及失败补偿统一进入 `CommitCanvasLayoutUseCase`，通过 `CanvasLayoutCommitPort` 调用各对象公开的位置入口。移动批次必须等待全部写入完成才报告失败；补偿基线记录每个流程终端的原始已提交坐标，不用流程外框平移代替恢复。位置恢复失败后仍继续尝试恢复已经移除的关系，聚合报告补偿失败，不宣称必然恢复成功。Presentation 对整理动作使用同步在途保护；工作区切换、卸载或用户手动移动视口后，旧请求可以完成原工作区的数据提交，但不能接管新视口。
...
| Presentation   | [`presentation`](../../../src/contexts/canvas-arrangement/presentation)、[`workbenchCanvasArrangementSelection.ts`](../../../src/presentation/app-shell/projections/workbenchCanvasArrangementSelection.ts)、[`useCanvasArrangementActions.ts`](../../../src/presentation/app-shell/coordinators/useCanvasArrangementActions.ts) |
