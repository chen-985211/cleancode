...
| 停止流程         | 停止依赖图       | 当前工作区活动工作流   | workflow run   |
...
“停止当前命令”只向一个终端的当前 PTY 发送 Ctrl+C；“停止流程”则停止整个活动 `WorkflowRun`，包括尚未开始和已经运行/就绪的节点。两者不得复用同一个作用范围。
...
- 普通终端运行环境动作进入 Run 的 `TerminalSessionService`。
