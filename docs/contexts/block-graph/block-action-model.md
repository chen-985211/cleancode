...
| 停止本次运行     | 停止依赖图       | 指定 `runId` 的活动工作流 | workflow run   |
...
“停止当前命令”只向一个终端的当前 PTY 发送 Ctrl+C；“停止本次运行”则携带根终端或活动通知绑定的精确 `runId`，停止该 `WorkflowRun` 中尚未开始和已经运行/就绪的节点，不影响同工作区其他活动工作流、同画布的无关命令、其他工作区或其他项目。两者不得复用同一个作用范围。
...
创建组合、加入或移出成员是第 4 条的前置结构动作例外：动作命中的任意终端必须通过共享契约扩展为当时依赖图中的完整弱连通流程，再以完整流程为单位调整成员。组合允许为空或只包含一个完整执行单元，成员移出不会自动解散组合。该扩展不改变普通单击、编辑终端或其他单终端动作的作用对象；组合启动仍只读取已经提交的精确成员，不在运行时再次沿连接扩大范围。
...
12. 同一工作区的无关流程不需要加入组合即可并发；任一终端同一时间最多属于一个活动 `WorkflowRun`。
