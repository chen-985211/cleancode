...
- 普通终端节点由 App Shell 组合 BlockGraph 定义与 Run runtime。BlockGraph snapshot 只在 adapter 中提供当前终端显示名、启动命令可用性和 block-scoped 动作绑定；Run 的 viewport、workflow 状态徽标和 runtime actions 不接收 BlockGraph snapshot，也不拥有节点、位置、尺寸或定义事实。编辑、删除和组合候选继续由 BlockGraph/App Shell 组合层发布。
- 终端定义表单与执行配置 draft 归 BlockGraph Presentation：它们编辑名称、描述、启动命令、任务/服务模式、就绪配置和持久化端口意图，并原子发布一个 `TerminalDefinitionInput`。表单不读取 Run session、端口租约或实际端点；App Shell 只把保存结果交给 BlockGraph 更新入口，并在节点上组合 Run runtime UI。
- 模板库与保存对话框归 BlockGraph Presentation，只消费 BlockGraph DTO、模板投影和显式的 list/save/update/move/delete 动作。App Shell `BlockTemplateSurfaces` 负责把 preload API 适配为这些动作，并继续协调画布放置、焦点和“放置后运行”；上下文组件不得直接读取 `window.cleancode` 或拥有 Run 执行结果。
- 模板放置的规范 footprint、外包围盒和纯预览归 BlockGraph Presentation：workflow 保留每个模板节点的相对位置，combination 投影为一个带规范 padding 和最小尺寸的组合边界。App Shell 只把实时 viewport 注入预览，并使用跨 Agent、终端与组合的工作台 occupancy 搜索最终空位；跨上下文碰撞策略不是 BlockGraph 单独事实。
...
| Presentation    | [`TerminalMetadataForm.tsx`](../../../src/contexts/block-graph/presentation/components/TerminalMetadataForm.tsx)、[`terminalExecutionConfigDraft.ts`](../../../src/contexts/block-graph/presentation/view-models/terminalExecutionConfigDraft.ts)、[`TerminalDefinitionPresentationTypes.ts`](../../../src/contexts/block-graph/presentation/view-models/TerminalDefinitionPresentationTypes.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
模板表现层入口为 [`BlockTemplateLibraryRoot.tsx`](../../../src/contexts/block-graph/presentation/components/BlockTemplateLibraryRoot.tsx)、[`BlockTemplateSaveDialog.tsx`](../../../src/contexts/block-graph/presentation/components/BlockTemplateSaveDialog.tsx)、[`BlockTemplatePlacementPreview.tsx`](../../../src/contexts/block-graph/presentation/components/BlockTemplatePlacementPreview.tsx)、[`BlockTemplatePresentationActions.ts`](../../../src/contexts/block-graph/presentation/view-models/BlockTemplatePresentationActions.ts) 和 [`blockTemplateGeometry.ts`](../../../src/contexts/block-graph/presentation/view-models/blockTemplateGeometry.ts)。
...
| Unit        | 终端定义表单、执行配置 draft、校验、焦点和原子保存输入               | [`block-graph.terminal-metadata-form.presentation.spec.tsx`](../../../tests/unit/contexts/block-graph/block-graph.terminal-metadata-form.presentation.spec.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
模板表现层由 [`block-graph.block-template-library.presentation.spec.tsx`](../../../tests/unit/contexts/block-graph/block-graph.block-template-library.presentation.spec.tsx)、[`block-graph.block-template-save-dialog.presentation.spec.tsx`](../../../tests/unit/contexts/block-graph/block-graph.block-template-save-dialog.presentation.spec.tsx)、[`block-graph.block-template-geometry.spec.ts`](../../../tests/unit/contexts/block-graph/block-graph.block-template-geometry.spec.ts) 和 [`block-graph.block-template-placement-preview.presentation.spec.tsx`](../../../tests/unit/contexts/block-graph/block-graph.block-template-placement-preview.presentation.spec.tsx) 证明筛选、维护、放置意图、保存输入、规范 footprint、viewport 投影、焦点和受控退出。
