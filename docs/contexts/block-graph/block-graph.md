...
6. 删除终端时同时删除关联连接，并从组合移除；不足两个顶层执行单元的组合自动消失。
...
1. 创建结果必须符合画布语义契约：至少包含两个顶层执行单元；单个终端或单条完整流程都不能形成组合。
...
5. 组合边界由成员位置、尺寸与固定留白计算，而不是独立自由缩放事实。
6. 移动组合会按位移量整体移动全部成员，再重新计算边界。
7. 移除完整执行单元后少于两个顶层执行单元，组合自动解散。
...
2. 多个执行单元使用每个终端的真实尺寸和固定 `64` 画布单位间距，在候选列数中选择接近横向 `2:1` 的紧凑排布；因此少量独立终端优先形成平衡网格，已连接工作流不会被无关终端插入依赖层。完整组合仍按派生外框形成避障视觉单元，与未组合终端及其他目标组合保留同等间距。
3. 整体排布边界以 `anchorRegion` 的水平中心为视觉锚点，并从其下方开始；作用域外终端、作用域外组合、anchor 和 `reservedRegions` 都是不可穿越的矩形障碍。避障按完整组合或完整依赖工作流的外接范围移动，不能逐个推移并拆散已经排好的空间关系。
...
`CreateTerminalBlockUseCase` 同样遵守该策略：显式 `position` 原样使用；省略位置时必须提供 anchor，并在创建终端的同一事务中自动落位。启动命令、尺寸、创建和自动落位作为一个提交完成。用户从主画布显式新建终端时，Presentation 必须通过跨终端、组合与 Agent 的统一创建协调策略提交显式 `position`；省略位置加 anchor 只服务 Agent 画布工具的确定性终端布局语义，不是用户界面创建失败时的备用落位公式。

`CreateTerminalWorkflowUseCase` 是 Agent 完整新建路径的原子边界。一个命令用调用内 `ref` 提供全部终端定义、完整执行配置、内部依赖和可选组合，并同时提供 anchor 与 reserved regions；同一默认图事务依次完成引用校验、创建、配置、连接、组合、确定性布局和工作流计划校验。任一输入、组合资格、依赖、配置、布局或计划失败都不得提交部分终端或连接。成功结果返回精确的新终端、连接、可选组合身份、实际排列作用域和已验证计划；既有细粒度用例继续承担已有图对象的增量编辑。
...
当前文件系统仓储按 `projectId + workspaceId` 把工作区默认图保存为 Electron 当前状态根中的版本 `3` JSON，并通过临时文件、同步和重命名原子替换。项目目录不是应用状态 owner，仓储不会读取项目内 `.cleancode/workspaces/.../default-graph.json`。
...
仓储写入版本 `3`，并只兼容读取版本 `2` 与 `3` 的规范快照。读取版本 `2` 时在内存中补齐五个空快捷位，不因读取本身回写文件；下一次真实图事务随其他变更写为版本 `3`。版本 `3` 必须包含顺序固定的五个位置，允许其中引用已经失效的对象；无版本、版本 `1`、未知版本、畸形端口意图或其他多余/缺失字段必须以稳定错误拒绝，不能静默修复或回写。

恢复仍会规范化兼容范围内的 viewport、尺寸、连接和组合：丢弃无效连接、重复连接、环、失效组合成员、重复归组、半流程组合和不足两个顶层执行单元的组合。规范化只移除无效组合关系并保留终端与连接，是兼容输入的边界；表现层不得自行修补聚合事实。
...
| Platform        | [`blockGraphIpcHandlers.ts`](../../../src/platform/electron-main/blockGraphIpcHandlers.ts)、[`blockTemplateIpcHandlers.ts`](../../../src/platform/electron-main/blockTemplateIpcHandlers.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Integration | 初始化、RMW、回滚、v2→v3 迁移与旧路径隔离                            | [`block-graph.filesystem-repository.spec.ts`](../../../tests/integration/contexts/block-graph/block-graph.filesystem-repository.spec.ts)、[`block-graph.store-versioning.spec.ts`](../../../tests/integration/contexts/block-graph/block-graph.store-versioning.spec.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
