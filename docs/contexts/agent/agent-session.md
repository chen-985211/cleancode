...
- `AgentSession` 聚合表示可持久化的工作区 Agent：稳定 `agentId`、名称、画布布局和各 Git 分支的 Codex thread 绑定。
- Agent 运行时会话表示当前进程中的一次 Codex PTY：易失 `sessionId`、进程号、状态、MCP 端点、Token、回调和待审批请求。
...
| 运行时会话   | 一个当前 Codex PTY 及其独立 MCP、审批和输出回调                    |
...
4. 为本次运行生成独立 `sessionId`，注册独立 MCP URL 与 Bearer Token。
5. 启动 Codex PTY，并在存在 UUID 时使用正式 resume 参数。
...
| Unit        | attach/restart、审批隔离、挂起恢复和运行时清理     | [`agent.session-service.spec.ts`](../../../tests/unit/contexts/agent/agent.session-service.spec.ts)                                                                                                                                |
