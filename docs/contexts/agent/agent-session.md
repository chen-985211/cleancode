...
每个工作区允许有零个或多个 Agent。首次初始化从未存在过的工作区时，应用层原子写入已初始化的空 Agent 列表，不检测 Provider 或创建默认 Agent。仓储把“检查是否已经初始化”和“写入空列表”作为同一个串行操作，因此并发首次列出只初始化一次。已经初始化为空的工作区不会在以后仅因 CLI 安装状态或偏好变化而自动补建；只有显式创建命令或用户配置的工作区默认内容初始化可以加入 Agent，用户删除最后一个 Agent 后重新打开同样保持零个。
...
4. session ref 必须由固定 Provider 的 codec 校验 kind、版本和值。Codex 注入正式 `tui.terminal_title` 配置，并把 Run 解析出的结构化 OSC title metadata 作为当前 thread 事实：未命名 thread 直接报告完整 UUID；命名 thread 的截断 UUID 只能由同一 executable 和环境下的正式 `app-server thread/list` 唯一前缀匹配补全，零命中或多命中都不得持久化。完成回合 notify 与精确信任的 `SessionEnd` Hook 只作兼容补报；已经观察到当前 title 后，其他 thread 的退出事件不得覆盖它。Claude Code 的 fresh `SessionStart(source=startup)` 和 `/clear` 后的 `SessionStart(source=clear)` 只表示 CLI 已分配 UUID，不证明本地 transcript 已形成，因此必须等 `UserPromptSubmit` 后才确认 session ref；`SessionStart(source=resume)` 与 `SessionStart(source=compact)` 指向已有可恢复对话，可以立即确认。缺失或未知 `source` 必须失败关闭，不能建立绑定。OpenCode 使用顶层 `session.created`、`chat.message` Hook 或恢复时 SDK `session.get` 确认的正式 `ses_` session ID，并排除带 `parentID` 的子会话；Gemini 使用 cleancode 通过正式 `--session-id` 参数预分配的 UUID，并通过 `SessionStart` Hook 跟进 CLI 内部恢复后的当前 session UUID。Pi 使用 extension 报告的精确 session 文件路径，确认文件已保存后通过 `--session` 恢复；Hermes TUI 使用本次 gateway 的结构化响应，把前台选择的临时运行 ID 映射为持久 session ID，并通过 `--resume` 恢复。所有 Provider 都只通过正式恢复参数恢复。系统不扫描 Provider 历史目录、可见终端文本或“最近会话”。
...
文件系统仓储当前写入 schema v6，兼容读取 v5；读取 v5 不回写，下次真实写入时保存为 v6。每个 Agent 只有一个 `providerSessionRef`。更早 schema、未知 session ref 版本或畸形数据必须拒绝读取，不迁移或回写；产品尚未公开期间由新的应用状态代际生成全新数据。cleancode 不复制或解析 Provider 对话正文。
...
工作区默认内容通过既有 `CreateWorkspaceAgentUseCase` 创建新身份、固定 Provider 和新会话，不复制其他 Agent 的 session ref 或自动发送提示词。内部初始化项 ID 与 Agent 创建凭据在 schema v6 的同一次 Agent 仓储写入中提交；凭据在 Agent 被删除后继续保留，用于拒绝重复创建。首次列表查询仍只初始化空列表，不能据默认配置创建 Agent。Provider 可用性、停用偏好和 Project 作用域校验沿用正常创建流程。
...
| Pi          | 是   | 是       | 状态与完成     | 否            | 临时 extension、已保存 session 文件的精确路径、`--session`                                                       |
| Hermes      | 是   | 是       | 状态与完成     | 否            | 本地 TUI gateway 结构化身份与前台选择、首次输入确认、`--resume`                                                  |

Pi 的 `session_start`（兼容旧版 `session_switch/session_fork`）、消息落盘后的采样与退出事件共同跟随当前 session 文件；文件路径由 extension 的 SessionManager 提供，只检查该精确文件是否已保存，不扫描目录或读取对话正文。新 Pi 会话在首条 assistant 消息落盘前保持 unbound。Hermes 的临时 Node preload 只观察当前 TUI 启动的 `tui_gateway.entry` 私有 JSON-RPC 与 `HERMES_TUI_ACTIVE_SESSION_FILE` 前台选择文件：`session.create` 的临时 ID 通过 `stored_session_id` 映射，在成功的 `prompt.submit` 返回 `streaming` 后确认；`session.resume` 的 `resumed` 与 `session.activate` 的已保存会话直接跟随。`/branch`（`/fork`）通过成功的 `session.branch` 响应和 `session.info` 事件中的 `stored_session_id` 确认分支身份，但成功响应不等于 TUI 接受切换：只有最新分支请求的成功响应，在同一次响应的 Promise 回调阶段紧接着发出关闭对应原会话的 `session.close` 时，才确认前台切换。候选在其他请求、下一次事件循环或 gateway 清理时失效；被后续命令取消的回调、无关关闭及乱序旧响应不得改变恢复引用。关闭结果不作为切换条件，因为 TUI 发出关闭请求后立即切换。该路径不更新前台选择文件，因此桥接暂存已确认的前台分支选择，直到该文件下一次明确写入，同 ID 的重新选择也必须生效。分支失败或原选择已变化的迟到响应不得切换绑定；分支身份缺失时保持 unbound，普通 `session.info` 不单独证明草稿已保存。后台响应不得改变前台选择，不扫描 Hermes 历史数据库，也不持久化协议中的对话内容。恢复传入 `--no-restore-cwd`，保留当前 cleancode 工作区目录。

两者的会话身份经过 launch 私有、限长、带单调序号的临时文件回报；重复与过期回报被合并或拒绝，清理时先读取最后回报再释放资源。CLI 内切换到尚未保存的新会话通过可选 `onProviderSessionCleared` 通知清空原绑定，清空与绑定必须进入同一有序持久化通道。旧 launch 的临时资源和最终回报排空后才增加新 generation；用户选择“新对话”时也必须先完成该排空，再清空原绑定，避免最终回报重新绑定旧对话。已释放 launch 的回调不得再修改绑定。Hermes 自动替换本地 gateway 时，桥接重新建立身份映射，并忽略旧 gateway 的迟到响应。Pi extension 与 Hermes preload 都只在本次 launch 注入，Hermes preload 不继续注入 gateway 的工具子进程，不修改用户全局配置。Hermes 此适配面向内建本地 TUI 的私有协议；自定义为非 TUI 命令、远程 gateway 或不提供这些元数据的版本不能据此推断身份捕获成功，未确认的会话不承诺恢复。

Pi 与 Hermes 的活动回报和会话身份回报使用独立通道。两种入口共用各自的结构化事件解释：托管 launch 通过带随机令牌、单调序号的私有 loopback reporter 调用现有 activity/completion 回调；普通终端通过既有 invocation 与 gateway 上报。发送队列有界、按序且请求超时后放弃积压，不因通知阻塞 CLI；托管启动经过普通终端 shim 时跳过第二次注入与回报。会话身份继续使用原有私有文件及有序持久化通道。

Pi 使用 `agent_start`、`ui_prompt_start/ui_prompt_end` 与 `agent_settled`。只有本次 run 最后 assistant 的 `stopReason=stop` 且 Pi 已停止自动继续时报告完成，`agent_end`、中间工具回合、自动重试本身不报告完成；未知确认弹窗只能表述为等待输入，不能猜成审批。该活动接口已按 Pi 0.85.0 验证；缺少这些事件的旧版本不承诺完整活动跟踪。Hermes 只投影当前前台 session 的 `message.start/message.complete`、`approval.request` 及带 request ID 的 `clarify/secret/sudo.request`；完成必须为 `status=complete`，审批回复按 `resolved` 数量清除，输入回复按 request ID 关联。多个等待未全部解决时保留等待状态，审批优先。错误、中断、会话切换与 gateway 释放收敛为 unavailable，不能借 working → idle 伪造完成。切回已有 live session 时，桥接从 `session.activate` 的 `running/status` 响应恢复该会话的运行状态，仅在前台选择文件确认后投影；响应自身不得改变前台选择。桥接在有界身份映射中按会话保留运行状态及未解决的输入、审批等待；切回时恢复等待投影且审批优先。后台回复、过期和结束同样清理对应等待，但不发布后台通知；迟到回复只清理其请求发出时关联的等待，不能覆盖新一轮请求。后续 `message.start/message.complete/error` 按会话更新它；后台结束或错误必须清除该状态，不能在切回后重放旧完成。后台事件、重复完成和旧 gateway 事件不产生新通知；Hermes 能力依赖内建本地 TUI 的对应私有协议。
...
普通终端使用 Agent activity 私有 zsh launcher 时，私有 `.zshrc` 在加载用户配置后调用环境中可选的 `CLEANCODE_ZSH_INPUT_INIT` 文件。这个进程级入口由 Run 提供并拥有按键兼容规则，Agent 只负责让默认启动链路执行同一初始化，不复制绑定、不修改用户 dotfile；入口未提供或文件不可读时继续正常启动。
...
原生消息通知由 `nativeMessages` capability 声明适配能力，但实际可用性按本次 CLI 版本、平台、启动参数与正式就绪信号决定。Agent 应用层为每个 launch 建立独立收件箱投递 lease；手动 attach、MCP 创建及恢复共用该机制。launch 替换、退出、MCP 撤销时先同步关闭 lease，再等待原生通知取消与 LIFO 资源清理。通知成功不改变消息确认事实，也不代表任务完成。Codex 的临时原生 launcher 在既有 PTY 中启动同环境的 TUI 与独立 app-server，并通过官方队列通知；Claude 使用不覆盖动态 watchPaths 的临时 FileChanged Hook；OpenCode 使用当前 native SDK 的 launch 级插件，手动新建与 MCP 创建通过正式初始提示建立同一类会话，恢复在插件初始化后异步确认。Gemini 尚无正式的空闲外部唤醒入口，保留主动领取。协议状态由[原生 MCP](cleancode-mcp.md)维护，版本、平台与真实验证记录见[兼容性报告](native-collaboration-compatibility.md)。
...
从活动状态回到 `idle`，或 Provider 显式报告回答完成时，Registry 按 invocation 分别进入短暂 quiet window；只有同一 invocation 重新变为 working/waiting 才取消自己的完成，其他 invocation 的活动不能吞掉它。Run 对同一完整 terminal generation 报告更大的已接纳输出 `sequence` 时，Registry 必须保留原 completion 身份并从该输出重新计算所有待发布完成的 quiet window；重复、倒退或旧 generation sequence 必须忽略，Agent 不读取或解析输出正文。quiet window 默认 1.5 秒，但每次完成从原始 `completedAt` 起最多等待 3 秒；输出只能重置静默期，不能推迟该截止时间，避免 TUI 状态栏持续刷新导致通知饥饿。静默期结束或达到截止时间时发布带稳定 `completionId` 的 `turn_completed`，同一 source revision 只能完成一次。两种完成来源、所有 Provider、托管和普通终端入口共用这一有界等待策略。显式完成只会把同一 invocation 的 working/waiting 收敛为 `idle`；从 `unavailable` 收到完成时仍保持 `unavailable`。Provider/PTY 退出本身不得伪造完成；invocation 退出会从快照移除该调用，terminal 退出或 generation 释放会终结该作用域。已由显式完成或转入 `idle` 开启的 quiet window 不因随后同一 invocation 的被动 unavailable/exit 丢失，但其更新的 working/waiting 仍会取消该完成，并使下一次完成拥有独立截止时间。应用重开时 Run 先恢复精确 terminal，再更新稳定 gateway manifest；没有发生在当前应用进程内的完成事件不从日志、输出或历史记录补造。
