...
- 为每个运行中的 Agent 会话注册独立 MCP 端点和 Bearer Token。
...
| 工具协议   | 工具名称、说明、输入 JSON Schema、审批属性和结构化结果组成的外部契约        |
...
| 责任                                 | Owner                   | 稳定协作方式                        |
| ------------------------------------ | ----------------------- | ----------------------------------- |
| 工具名称、说明和输入 Schema          | Agent application       | `AgentToolProtocol`                 |
...
会话启动顺序固定为：先注册 MCP 端点，再启动 Codex PTY。cleancode 通过进程参数写入 `mcp_servers.cleancode.url`，通过子进程环境变量 `CLEANCODE_MCP_TOKEN` 提供 Token，并让 Codex 从该环境变量读取 Bearer Token；该过程不修改用户的全局 Codex 配置。
...
| `tools/list`                | 返回当前工具名称、说明和输入 JSON Schema                     |
...
| Contract / tool protocol | 完整工具目录、严格 Schema 和排除的通用工具 | [`agent.tool-protocol.spec.ts`](../../../tests/contract/contexts/agent/agent.tool-protocol.spec.ts)                                                                                                |
| Contract / JSON-RPC      | 初始化说明、工具列表和调用结果结构         | [`agent.json-rpc-tool-bridge.spec.ts`](../../../tests/contract/contexts/agent/agent.json-rpc-tool-bridge.spec.ts)                                                                                  |
...
| Integration / Codex PTY  | URL、Token、开发说明和 MCP 配置注入        | [`agent.codex-pty-process.spec.ts`](../../../tests/integration/contexts/agent/agent.codex-pty-process.spec.ts)                                                                                     |

手工验收至少覆盖：

1. Agent 能先查看画布，再创建终端积木和终端组合。
