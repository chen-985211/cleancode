...
- 查看积木图，创建、更新、删除终端积木，创建、更新、解散终端组合，并在组合间原子移动完整终端流程。
...
启用能力时，顺序固定为：先注册 MCP 端点，再由当前 Provider contribution 生成 launch 级配置，最后在 Agent terminal 中启动 Provider CLI。Codex 使用进程级 `--config` 注入 `mcp_servers.cleancode`、Token 环境变量、默认批准模式和 developer instructions；Claude Code 使用 mode `0600` 的会话临时 MCP 文件、环境变量展开的 Authorization header、`--mcp-config`、`--allowedTools mcp__cleancode__*` 和 `--append-system-prompt-file`；OpenCode 合并用户已有的 `OPENCODE_CONFIG_CONTENT`，用 `{env:...}` header 引用 launch token，并注入临时 instructions 与 `file://` reporter 插件；Gemini 使用 mode `0600` 的 launch 临时 system settings，通过 `GEMINI_CLI_SYSTEM_SETTINGS_PATH` 注入 `mcpServers.cleancode.httpUrl`、引用 `${CLEANCODE_MCP_TOKEN}` 的 Authorization header 与 `trust: true`。Gemini 依赖官方 settings 层的对象合并语义保留用户和项目中的其他 MCP；四者都不得覆盖用户其他 MCP 或写入工作区/全局配置。

Claude Code 的完整 launch 指令存入 mode `0600` 的私有临时文件，所有平台均只向原生 CLI 传入文件路径，避免长指令超过 Windows npm `.cmd` 入口的命令行限制。用户已有的追加提示文本或提示文件内容与 CleanCode 指令合入本次 launch 文件，保留内容且不改写用户文件；不得同时向 CLI 传入两种互斥的 append 参数。临时文件由 `AgentLaunchArtifactScope` 随 launch 退出、替换或失败清理；关闭 MCP 时保留用户原始提示参数，不注入 CleanCode 指令。
...
未知方法返回 JSON-RPC `-32601`；未知工具名、非法调用外形或显式传入非对象 `arguments` 时返回 `-32602`。已识别工具的参数由 `tools/list` 暴露的同一份 Schema 在应用层递归校验，使用 `additionalProperties: false`、必填属性、联合类型、数值边界、数组长度和唯一性等约束；`create_terminal_workflow` 与 `create_terminal_set` 再由同一输入解析入口复用共享画布语义分类，分别只接受单一完整流程和多个独立顶层执行单元。每个 MCP `outputSchema` 的根明确声明 `type: object`，再以严格分支描述完成、失败和可选取消结果。输入失败返回 `AGENT_TOOL_INPUT_INVALID`，不得触达 BlockGraph 端口。工具输入、领域规则或执行失败仍属于合法的 MCP 工具结果：HTTP 保持 `200`，`isError` 为 `true`，并返回结构化错误。未知异常统一净化为 `UNEXPECTED_ERROR`，不得暴露原始异常文本或堆栈。

进程级 `developer_instructions` 与 `initialize.instructions` 共同承担开启 CleanCode MCP 时的画布语义消歧：二者先投影共享画布语义契约生成的同一规范说明，再追加各自的路由和能力边界。用户未加限定地说“终端”“整理终端”“终端布局”“终端组合”“终端工作流”“启动项目的终端组合”或同义请求时，Agent 必须先调用 `inspect_graph`，并默认把作用对象理解为 CleanCode 画布事实。Agent 可以在查看画布后读取仓库以确认真实启动单元；新建结构必须按规范分类一次调用：一个已配置终端使用 `create_terminal`，一条至少包含两个依赖连通终端的完整流程使用 `create_terminal_workflow`，多个独立顶层执行单元使用 `create_terminal_set`。用户明确要求流程时不得静默降级到另外两个工具；启动项目流程可以把仓库真实支持的依赖安装或初始化作为上游任务、开发服务作为下游服务，若仍无法识别两个真实阶段则先询问用户，不能只创建终端，也不能虚构等待或转发步骤。`terminalGroup` 默认省略，只有用户明确要求终端组合、容器或分组时才提供；多个相关终端或依赖关系本身不构成组合意图。要把既有完整流程纳入新组合时，也只有在用户明确要求组合时才先用 `create_terminal_group` 创建空组合，再用 `move_terminal_workflow_to_group` 移动流程；不要通过兼容字段 `memberBlockIds` 重新拼装既有流程。`create_block`、`update_terminal_execution_config`、`connect_terminal_blocks`、`create_terminal_group`、`move_terminal_workflow_to_group`、`arrange_terminal_layout` 和 `inspect_terminal_workflow_plan` 继续服务空白视觉终端或既有画布对象的增量编辑。不得把直接运行 Shell 进程、创建 package script、`.vscode` task 或项目配置当成替代品。当前工具目录只能创作和检查工作流，没有启动工具，因此 Agent 只能声明已经搭建，不能声称终端或工作流已经运行。只有用户明确提到“终端源码”、`Terminal component`、xterm、PTY 或终端模块实现等源码限定词时，才把请求理解为项目代码工作。关闭 MCP 时不提供该 Server 及其 instructions，也不注入这层进程级画布语义。
...
| `create_terminal`                  | 原子创建、配置、排列并校验一个终端             | `name`、`launchCommand`            | `executionConfig`、明确意图时的 `terminalGroup`            | 否      |
| `create_terminal_workflow`         | 原子创建、配置、连接、排列并校验一条完整流程   | `terminals`、`connections`         | 明确组合意图时的 `terminalGroup`                           | 否      |
| `create_terminal_set`              | 原子创建并校验多个独立顶层执行单元             | `terminals`、`connections`         | 明确组合意图时的 `terminalGroup`                           | 否      |
| `update_block`                     | 更新终端积木元数据、位置或大小                 | `blockId`                          | `name`、`description`、`launchCommand`、`position`、`size` | 否      |
| `delete_block`                     | 删除终端积木                                   | `blockId`                          | 无                                                         | 是      |
| `create_terminal_group`            | 创建视觉组合；省略位置的空组合会安全落位       | `name`                             | `memberBlockIds`、`position`                               | 否      |
| `move_terminal_workflow_to_group`  | 把既有完整流程加入、转移或移出组合             | `blockId`、`targetTerminalGroupId` | 移出时必须提供 `position`                                  | 否      |
...
`0.4.0` 新增 `create_terminal_workflow`。输入用一次调用内的稳定 `ref` 描述终端、内部连接和仅在用户明确组合意图时提供的可选组合；应用层把当前工作区全部 Agent 的已保存布局作为无优先级 `canvasRegions` 注入，BlockGraph 再与图内顶层对象共同寻找空位。BlockGraph 在一个仓储事务中完成定义校验、终端创建与完整执行配置、内部依赖、可选组合、确定性排列和计划校验；任一环节失败都不提交图。成功结果返回 `ref` 到持久化终端 ID、连接 ID、可选组合 ID、实际排列作用域和已验证计划，并以一次已提交图变更为 Presentation 提供连续演出事实；演出可以逐步或整体呈现，但不得把视觉阶段重新拆成多次工具调用。原有细粒度工具不删除、不改名，继续用于既有图的增量编辑。

`0.5.0` 新增 `move_terminal_workflow_to_group`，用一个代表终端把其完整弱连通流程原子加入、转移或移出组合；加入和转移使用目标组合的固定空间锚点自动排版，移出要求新的顶层 `position`，目标作用域与当前作用域相同时返回真正的无变化结果。应用层注入当前工作区全部 Agent 的权威布局，BlockGraph 用最终完整组合外框或移出后的全部流程终端检查 Agent、外部顶层终端和外部组合；有冲突时整体回滚，不能移动组合锚点来掩盖冲突。`create_terminal_group` 现在允许只提供 `name` 创建空组合，省略 `position` 时按同一顶层避障事实安全落位；`memberBlockIds` 只为兼容既有调用保留，不再是推荐的既有流程组装入口。组合创建和成员移动成功时返回精确排列对象 ID，供 Presentation 做一次局部聚焦。

`0.6.0` 把原子创建入口按规范结构拆为 `create_terminal`、`create_terminal_workflow` 和 `create_terminal_set`。三者复用同一个 Agent 到 BlockGraph 原子创建端口与事务，但外部 Schema 不再允许模型自报结构：流程工具至少需要两个终端和一条依赖，并由 Agent 应用层继续验证全部 refs 只形成一个完整连通图；集合工具只接受多个独立顶层执行单元；单终端工具没有连接输入。错配以 `AGENT_TOOL_INPUT_INVALID` 失败且不触达 BlockGraph。成功输出仍回显 `structureType`，使 Agent 必须按实际结构说明结果。
...
Agent 使用画布工具时应先调用 `inspect_graph` 获得当前 ID、配置和依赖，再执行后续变更。完整新建路径按结构选择 `create_terminal`、`create_terminal_workflow` 或 `create_terminal_set`，在一次提交中返回新对象 ID、排列结果和已验证计划，不需要再串联细粒度工具；只有用户明确要求把既有流程组成新组合时，才使用空 `create_terminal_group` 加 `move_terminal_workflow_to_group`，不要优先使用兼容的 `memberBlockIds`。既有对象编辑路径仍按实际需要使用细粒度工具并在需要时重新排列、检查计划。Agent 的最终说明应报告持久化的策略与注入方式，不能把请求端口误报为本次运行的实际端点。
...
| 工具                   | 输入与结果                                                                                                       |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `list_agents`          | 无输入；返回自身 ID 和当前工作区全部持久化 Agent，包括视口外对象、MCP 开关、是否存在消息等待和 `deliveryStatus`  |
...
消息种类为 task、question、progress、result。关联回复只能回给一条发给自己的消息的发送者；向第三个 Agent 交接使用独立消息，不携带原消息的 `replyToMessageId`。这个关联约束不建立汇报层级，也不强制每条消息都产生回复。相同 message ID 与相同内容重试复用原消息，不重复投递；改写内容或冒用其他发送者的 ID 会失败。接收方接受消息后在下一次等待中明确确认，未确认消息可以再次读取。该语义保证至少一次交付，不保证任务只执行一次；Agent 应按 message ID 避免重复副作用。确认不表示任务成功；需要向同伴同步任务完成时，通过关联的 result 表达。
...
当前协作指引由 Agent 根据用户要求和任务依赖选择沟通对象。所有 Agent 都可以在自己的原生对话中直接向用户反馈结果、发现或需要用户决定的问题；创建关系和任务发送关系不指定固定负责人。用户明确要求某个 Agent 汇总时遵守该要求，否则选择能够推进任务且避免重复转述的沟通路径。交接应说明目标、必要上下文与约束，以及谁需要结果继续工作；优先复用适合的已有 Agent，新建时给出具体初始任务，共享文件的重叠修改应先协调。

如果同伴仍依赖结果、明确请求回复，或需要根据发现和阻塞采取行动，应发送必要的关联 result、question 或 progress，并包含证据和下一步。直接向用户反馈不能替代解除已有协作依赖所需的回复；没有同伴需要结果时，可以直接向用户反馈并结束。必要的同伴更新应说明是否已经告知用户或正在等待用户决定，接收方避免重复汇报、重复提问和逐级转发；用户要求汇总或存在实质后续动作时可以继续处理。纯领取、确认、空收件箱、启动就绪和礼貌性回应不产生额外汇报或回复，除非用户明确要求；消息领取确认已表达接收，不通过回复套回复确认。

以下场景使用同一规则，不因 Provider 或手动/MCP 创建入口而改变：

| 任务关系                     | 预期沟通结果                                                                                   |
| ---------------------------- | ---------------------------------------------------------------------------------------------- |
| Review 发现代码需要修改      | 将可操作的发现和证据告知负责修改的 Agent；已告知用户时注明，避免再转述一次                     |
| 独立任务，没有同伴依赖其结果 | 执行 Agent 可以直接向用户反馈，无须固定回传创建者                                              |
| 需要用户决定且同伴工作受阻   | 执行 Agent 直接询问用户，并向受影响同伴同步阻塞和已询问状态；同伴不再次提问                    |
| 用户明确要求某个 Agent 汇总  | 向该 Agent 提供汇总所需结果，由它完成用户要求的汇总                                            |
| 仅创建成功、就绪或消息已领取 | 使用已有启动状态或收件箱确认，不生成额外寒暄、就绪转述或确认循环；用户明确请求检查时按要求反馈 |

上述沟通选择属于模型指引，不是服务端新增的任务调度器或已读用户状态。服务端仍只保证鉴权、消息身份、关联、确认与投递；不能据此保证模型每次都选到最合适的反馈对象。
...
- `structuredContent`：遵循每个工具公开的 `outputSchema`。成功图工具包含 `status`、稳定的 `toolCallId`、`graphChanged`、工具输出和最新图快照；原子创建结果额外回显规范 `structureType`；计划检查返回计划而不复制图快照；失败返回结构化 `error`。
- `isError`：完成时为 `false`；取消或失败时为 `true`。`awaiting_approval` 是应用内部状态，实际 MCP 调用会保持等待，不把它作为最终外部结果返回。

只有 `graphChanged: true` 的成功变更才通过回调把最新图快照通知表现层；`inspect_graph`、计划检查和幂等布局不得触发伪更新。显式布局、组合创建和组合成员移动成功时，布局事件携带 `terminal_layout_arranged`、原始 `toolCallId` 和实际排列的终端/组合 ID。单个 `create_block` 成功与三个原子结构创建工具成功都发布统一的 `terminal_build_created` 事件；前者携带单个终端 ID，后三者携带本次终端、连接与明确请求的组合 ID，并保留原始 `toolCallId`。表现层只能在收到这份已提交事实后先聚焦、再按用户偏好演出，不得用动画预告未提交修改。搭建层只安排出现时机、起始位置、终端顺序和连线阶段；终端、组合、连线与相机的具体效果由画布对象既有原子 owner 负责，MCP 来源不得引入第二套创建动效。事件元数据和聚焦/搭建阶段不持久化，也不改变 BlockGraph 事实。
...
| Presentation projection       | [`agentApprovalPresentation.ts`](../../../src/presentation/app-shell/projections/agentApprovalPresentation.ts)、[`useAgentLayoutCoordination.ts`](../../../src/presentation/app-shell/coordinators/useAgentLayoutCoordination.ts)、[`terminalWorkflowBuildChoreography.ts`](../../../src/presentation/app-shell/projections/terminalWorkflowBuildChoreography.ts)、[`useTerminalWorkflowBuildChoreography.ts`](../../../src/presentation/app-shell/coordinators/useTerminalWorkflowBuildChoreography.ts)、[`useWorkbenchLayoutFocus.ts`](../../../src/presentation/app-shell/workbench/viewport/useWorkbenchLayoutFocus.ts) |
...
| Contract / JSON-RPC         | `0.8.0` 初始化、执行结构判别、同源画布语义、原子工作流与组合成员移动路径、自描述端口策略、稳定调用 ID 和结构化/净化错误                 | [`agent.json-rpc-tool-bridge.spec.ts`](../../../tests/contract/contexts/agent/agent.json-rpc-tool-bridge.spec.ts)、[`agent.tool-protocol.spec.ts`](../../../tests/contract/contexts/agent/agent.tool-protocol.spec.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
1. MCP 开启时，Agent 对“帮我搭一个前端依赖后端的终端工作流”先查看画布，可以再读取仓库确认启动命令，随后一次调用 `create_terminal_workflow` 提交至少两个终端组成的完整依赖连通图；明确流程请求不得改用 `create_terminal` 或 `create_terminal_set`，无法确认真实阶段时必须先询问。单启动单元请求使用 `create_terminal`，多个独立执行单元请求使用 `create_terminal_set`；不得拆分调用、虚构步骤、用直接启动 Shell 进程代替，或把已搭建表述为已运行。
...
9. Agent 完成终端工作流后只排列精确相关终端；提交成功后优先从发起 Agent 边缘连续演出，逐步模式按稳定拓扑顺序逐个搭建，整体模式让终端同时进入，节点接近落位后连线、明确请求的组合最后收束，启动窗口分别不超过 8s 与 1.4s；工作流创建在演出开始时用一次可中断聚焦同时框住发起 Agent 和本次最终可见结果，聚焦最大为 `100%` 且在范围较大时自动缩小，组合创建或成员移动只局部框住结果。搭建只编排时机、起点、顺序和连线，节点出现复用画布原子创建动效。聚焦复用弹簧控制器，用户平移/缩放立即取消，新图更新中断并接续最新事实，reduced-motion 直接显示最终布局。
