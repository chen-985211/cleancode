...
读取经 `GitHubIssuePort`，当前适配器使用已安装且登录的 GitHub CLI，仅支持 github.com。所有查询都在已登记的项目根目录执行。开始处理前重新校验项目登记和仓库选择，再向 GitHub 读取 Issue；已有关联时选择该工作区。没有关联时，由 `IssueWorkspaceBasePort` 校验基准分支、读取其远程提交并拉取到本地。基准默认是该仓库的默认分支，也允许用户明确指定；新 worktree 从冻结提交创建，不依赖当前 checkout 的分支，不切换原物理工作区。

读取权限以 GitHub 的实际响应为准，不按仓库所有者过滤。仓库关闭 Issues、凭据失效、权限不足、资源不可访问和限流分别返回稳定错误；仓库已识别但列表失败时保留其名称作为错误上下文，允许用户更换来源，不把失败当作空列表。

Issue 创建复用普通分支工作区的默认内容与初始化流程，冻结请求同时保存 `issue` 和 `baseRef`。相同项目的开始请求串行，未完成请求重用原请求 ID、工作区 ID、分支名与基准提交；重试不拉取新基准，也不再次创建已经成功的内容。Git 已成功但 Project 保存失败时，沿用创建凭据恢复规则，并为同步发现的工作区补回 Issue 引用。读取与刷新不创建工作区，不执行 Issue 正文，也不写回 GitHub。
