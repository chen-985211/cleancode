...
读取经 `GitHubIssuePort`，当前适配器使用已安装且登录的 GitHub CLI，仅支持 github.com。所有查询都在已登记的项目根目录执行。开始处理前重新校验项目登记和仓库选择，复用主进程从列表或详情读取的 Issue 引用；有界内存缓存按项目身份、目录、来源仓库隔离，不接收 Renderer 提交的 Issue 内容，也不持久化正文。缓存缺失时可复用已保存的工作区或未完成请求引用，否则向 GitHub 读取。已有关联时选择该工作区。没有关联时，由 `IssueWorkspaceBasePort` 校验并解析基准：本地分支或完整提交已存在时直接使用，不查询网络；明确的远程分支（如 `origin/main`）拉取对应远程跟踪引用，本地没有同名分支时才按来源仓库获取远程基准。基准默认是该仓库的默认分支，也允许用户明确指定；新 worktree 从冻结提交创建，不依赖当前 checkout 的分支，不切换原物理工作区。
