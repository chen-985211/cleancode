...
Issue 创建复用普通分支工作区的默认内容与初始化流程，冻结请求同时保存 `issue` 和 `baseRef`。相同项目的开始请求串行，未完成请求重用原请求 ID、工作区 ID、分支名与基准提交；重试不拉取新基准，也不再次创建已经成功的内容。Git 已成功但 Project 保存失败时，沿用创建凭据恢复规则，并为同步发现的工作区补回 Issue 引用。打开创建浮层或修改基准时可提前解析基准；预取不创建分支或工作区。相同项目目录、来源及基准的远程拉取合并在途请求，成功结果最多复用 30 秒，失败不缓存；正式创建始终重新校验项目身份和来源，远程更新失败不静默退回旧提交。主进程按请求报告“准备基准”和“创建”阶段，进度只发给发起窗口；窗口关闭不影响创建结果。读取与刷新不创建工作区，不执行 Issue 正文，也不写回 GitHub。
