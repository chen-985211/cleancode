...
- 删除终端、checkout、归档 worktree、同步失效工作区和移除项目都会进入 Run 硬清理；应用退出只硬清理默认策略会话与工作流。
...
- Platform 应用退出先阻止新 Run 启动并停止活动工作流；默认策略会话继续硬清理。只有用户明确保留、Provider checkpoint 可靠且非工作流的普通/直接启动会话可以 detach，应用重开后仍按监听所有权规则重建端点租约。
