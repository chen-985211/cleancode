...
Provider 协议 v6 沿用 v5 的控制权、`terminalSourceTheme`、结构化 `launchForegroundJob`、started/exited 定向事件与两阶段应用 detach，并把“普通 PTY 不继承 Provider 的 Electron Node 模式”纳入兼容边界。客户端只连接 v6 Provider；发现仍存活的 v5 Provider 时以 `TERMINAL_PROVIDER_PROTOCOL_UNSUPPORTED` 失败关闭，不复用不满足环境边界的旧进程，也不为升级杀死其中仍被用户保留的普通终端。旧 Provider 自然结束后，后续应用启动按陈旧 metadata 流程创建 v6 Provider。
