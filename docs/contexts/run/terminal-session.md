...
每个模型使用与可见终端一致的行列和 `scrollback: 1000`，维护 ANSI 屏幕、光标、颜色、alternate buffer、已支持模式、标题、工作目录和可读 transcript。`TerminalSnapshot.sequence` 与 `RestoreMarker` 表示快照已经包含到哪个输出；renderer 只接受同一 `viewId` 和完整运行身份的后续事件，并按序丢弃重复、检测缺口。
...
5. `ReplaceSession`、显式终止和统一硬清理同时释放 PTY、模型、视图租约与缓冲；自然退出只在当前应用进程内保留有界最终模型。
...
普通 `TerminalSession`、PTY 进程、输入历史、权威终端模型、输出序号、视图租约、端口租约和实际端点都是当前进程内的易失状态，不写入仓储。renderer 重建可以从仍存活的主进程模型恢复；应用完整退出后仍由已持久化的 BlockGraph 终端配置重新建立空会话或启动命令，不恢复旧进程、模型、输入历史、PID 或端口分配。
...
| Application    | [`TerminalSessionService.ts`](../../../src/contexts/run/application/use-cases/TerminalSessionService.ts)、[`LaunchTerminalCommandUseCase.ts`](../../../src/contexts/run/application/use-cases/LaunchTerminalCommandUseCase.ts)、[`RunLifecycleService.ts`](../../../src/contexts/run/application/use-cases/RunLifecycleService.ts)、[`TerminalProcessPort.ts`](../../../src/contexts/run/application/ports/TerminalProcessPort.ts)、[`TerminalModelPort.ts`](../../../src/contexts/run/application/ports/TerminalModelPort.ts) |
| Infrastructure | [`NodePtyTerminalProcessAdapter.ts`](../../../src/contexts/run/infrastructure/pty/NodePtyTerminalProcessAdapter.ts)、[`HeadlessTerminalModelAdapter.ts`](../../../src/contexts/run/infrastructure/terminal-model/HeadlessTerminalModelAdapter.ts)                                                                                                                                                                                                                                                                              |
...
| Unit / Service | 精确作用域、替换、迟到事件、sequence、视图 attach/detach、Ctrl+C、异步关闭、启动校验与 lifecycle gate | [`run.terminal-session-service.spec.ts`](../../../tests/unit/contexts/run/run.terminal-session-service.spec.ts)、[`run.terminal-session-model-lifecycle.spec.ts`](../../../tests/unit/contexts/run/run.terminal-session-model-lifecycle.spec.ts)、[`run.run-lifecycle-service.spec.ts`](../../../tests/unit/contexts/run/run.run-lifecycle-service.spec.ts)、[`run.launch-terminal-command.spec.ts`](../../../tests/unit/contexts/run/run.launch-terminal-command.spec.ts) |
| Integration    | 真实 node-pty 与 headless 模型的 ANSI、模式、alternate buffer、背压、进程清理、退出和工作目录         | [`run.pty-terminal.spec.ts`](../../../tests/integration/contexts/run/run.pty-terminal.spec.ts)、[`run.headless-terminal-model.spec.ts`](../../../tests/integration/contexts/run/run.headless-terminal-model.spec.ts)                                                                                                                                                                                                                                                       |
| Contract       | snapshot、restore marker、视图身份、定向输出和 renderer 销毁清理                                      | [`run.terminal-view-ipc.spec.ts`](../../../tests/contract/contexts/run/run.terminal-view-ipc.spec.ts)                                                                                                                                                                                                                                                                                                                                                                      |
...
| E2E            | 用户启动、输入、关闭、真实视图重建、隐藏输出恢复和 query 唯一响应                                     | [`run-terminal-sessions.e2e.spec.ts`](../../../tests/e2e/run-terminal-sessions.e2e.spec.ts)、[`git-branch-workspaces.e2e.spec.ts`](../../../tests/e2e/git-branch-workspaces.e2e.spec.ts)                                                                                                                                                                                                                                                                                   |
