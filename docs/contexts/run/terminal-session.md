...
Provider 协议 v5 沿用 v4 的控制权、`terminalSourceTheme`、结构化 `launchForegroundJob` 与 started/exited 定向事件契约，并把应用 detach 拆为 `beginApplicationDetach` 与 `awaitApplicationDetach`。前者先让 Provider 进入 `releasing`、建立单次 release 并返回不透明的 `releaseId` receipt；后者在同一条已认证连接上等待该 release 的最终结果。客户端连接紧邻的 v4 Provider 时回退到旧 `detachApplication`，仍等待同一释放语义，不为升级杀死仍持有普通终端的进程。
