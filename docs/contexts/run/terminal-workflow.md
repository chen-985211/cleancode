...
| 工作流范围 | 本次计划包含的终端集合；当前 UI 使用“指定终端及其后代”或“精确组合成员” |
...
当前 UI 可以从终端节点的“从此处运行流程”或组合的“启动组合命令”发起，不提供空闲态全局运行按钮。普通“启动命令”只运行当前终端，不触发依赖级联。组合启动通过一次工作流请求提交成员作用域，表现层不得逐成员直接启动；同一组合中的独立根和不同流程可以并行，但每条流程仍按直接依赖推进。
