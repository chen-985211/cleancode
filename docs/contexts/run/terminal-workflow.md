...
- 在同一工作区分别启动并同时保留节点集合互不重叠的多个活动运行，组合不是并发前提。
...
| 阻塞       | 由于某个上游失败而不会再运行                                                             |
| 停止       | 用户按运行 ID 停止活动工作流后，运行中、就绪或尚未开始的节点进入的终止结果               |
...
活动运行以项目 ID/目录、工作区名称/目录和 Git 分支作为权威作用域，并以“项目目录 + 工作区 ID + 运行 ID”索引具体 `WorkflowRun`。同一项目的同一工作区可以保留多个运行投影，但任意两个活动运行的终端节点集合必须互不重叠；不相交运行可以分别启动、查询和停止，不需要先放入终端组合。新计划与活动运行共享终端时，Run 在节点 owner 启动队列内原子拒绝新计划，不停止或替换已有运行；新计划只与旧终态投影重叠时，先通过硬生命周期收束该旧运行仍持有的 PTY、探测与端口资源，再用新运行替换这些节点的旧投影。不同项目即使都使用 `main` 工作区也必须彼此隔离，不能消费对方的运行事件。每个节点 PTY 继续使用完整 `sessionId + runId + generation` 隔离迟到事件。
...
同一项目的同一工作区可以同时保留多个活动工作流，只要它们的计划节点集合互不重叠。启动入口先生成不可变计划，再由 Run 应用服务按计划中的终端 owner 串行化注册：没有节点交集时独立运行；与活动运行存在交集时返回 `TERMINAL_WORKFLOW_SCOPE_CONFLICT`，已有运行保持不变；只与旧终态投影相交时，先硬清理该旧运行仍持有的 PTY、探测与端口资源，再移除旧投影并注册新运行。不同项目或不同工作区继续彼此隔离。

当前 UI 可以从终端节点的“从此处运行流程”、组合的“启动组合命令”、模板库的“放置并运行”、已配置工作区默认模板的“放置后运行”或工作区快捷执行键位发起，不提供空闲态全局运行按钮。快捷执行栏本身只展示映射，点击格子不得启动；`Command/Ctrl + 1` 至 `5` 才把 BlockGraph 拥有的对象引用交给既有入口。流程按绑定时精确成员调用既有 `block-set` 作用域，组合按组合 ID 调用既有组合入口，快捷执行不拥有或复制计划、调度与运行状态。普通“启动命令”只运行当前终端，不触发依赖级联。组合启动通过一次工作流请求提交成员作用域，表现层不得逐成员直接启动；同一组合中的独立根和不同流程可以并行，但每条流程仍按直接依赖推进。多个无关流程即使不在组合中也可以分别启动并发运行；组合只提供视觉组织和一次批量启动。模板运行必须使用实例化返回的精确作用域，不能重新从模板快照拼装或复制调度规则。

工作区默认模板的启动使用实例化返回的新对象作用域和目标 worktree 目录。Project 在调用前保存启动请求，Run 返回的运行 ID 只用于关联此次请求；启动响应未知时不自动重放，也不从初始化记录恢复已结束的运行。
...
每个活动工作流都在右上角拥有一条按运行 ID 区分的活动通知并提供“停止本次运行”，本次计划的根终端也将“从此处运行流程”切换为同名兜底动作；顶部工具栏不展示工作流状态或停止动作。“停止本次运行”必须携带精确 `runId`，只停止该 `WorkflowRun` 中尚未开始以及已经运行或就绪的全部节点，不影响同工作区其他活动流程、同画布无关的独立终端命令、其他工作区或其他项目。单个终端的“停止当前命令”只中断该终端当前 PTY，两者是不同作用范围的动作。
...
每个工作流开始后，表现层按运行 ID 创建一条可关闭的活动通知；同一工作区的多个活动运行分别占用通知堆栈中的独立条目。每条通知展示起点、精确终端数量和只绑定本次 `runId` 的“停止本次运行”，并在运行、服务就绪、成功、失败或已停止之间原地更新。所有工作流状态都复用应用通知的唯一固定宽高两行卡片：第一行显示状态，第二行显示起点与终端数量或失败摘要。通知正文可以显式定位本次运行在当前画布上的相关对象；Presentation 只消费当前运行快照，依次选择失败、运行、就绪节点或流程起点，并把来源节点保存为 `projectId + workspaceId + objectKind + objectId` 规范画布身份。定位前必须完整匹配当前 Workbench 身份，并在目标属于折叠终端组合时定位可见组合；定位选择并居中对象，在目标支持时激活主输入，但不停止流程或关闭通知，身份不匹配或目标已经不存在时不得跳转到其他对象。通知级可访问名称必须包含流程状态标题以及失败摘要或流程范围，正文按钮另行表达定位动作。停止动作使用独立停止图标进入卡片最右侧控制轨，位于关闭按钮之前，默认不显示背景或边框，只在悬停、聚焦或进行中投影必要反馈，并始终保留完整 Tooltip 与可访问名称。成功和已停止可以短暂显示后自动关闭；失败默认保留，通过错误语义色与图标区分，并使用失败终端名称和结构化退出码生成稳定用户文案，不直接展示未经映射的底层异常文本。同一次运行的终态重复更新以及工作区切换后重新加载的同一终态快照不得重复通知。用户主动关闭活动通知不得停止运行，后续普通状态不得重新打开它；返回仍在活动的工作区时必须恢复每个活动运行的通知和停止入口，如果某个运行随后失败或在离开期间首次失败，必须为该运行重新弹出一次错误通知。连接、配置、启动或停止流程等用户动作失败时同样进入应用通知，不在顶部工具栏中常驻显示错误。关闭通知只移除临时反馈，失败节点、阻塞后代、连线状态和终端输出继续保留。
...
| Presentation           | [`terminalWorkflowSessionEvents.ts`](../../../src/contexts/run/presentation/view-models/terminalWorkflowSessionEvents.ts)、[`TerminalRuntimeHeader.tsx`](../../../src/contexts/run/presentation/components/TerminalRuntimeHeader.tsx)、[`useTerminalWorkflow.ts`](../../../src/presentation/app-shell/coordinators/useTerminalWorkflow.ts)、[`useTerminalWorkflowNotifications.ts`](../../../src/presentation/app-shell/context-adapters/run/useTerminalWorkflowNotifications.ts)、[`terminalWorkflowNotifications.ts`](../../../src/presentation/app-shell/context-adapters/run/terminalWorkflowNotifications.ts)、[`terminalWorkflowEdges.ts`](../../../src/presentation/app-shell/projections/terminalWorkflowEdges.ts)、App Shell 终端节点组合与通知 |
...
| Unit / Run          | 状态迁移、并行、汇合、失败传播、节点占用冲突、精确停止、应用退出交接、实际端点和受管服务就绪    | [`run.workflow-run.spec.ts`](../../../tests/unit/contexts/run/run.workflow-run.spec.ts)、[`run.terminal-workflow-service.spec.ts`](../../../tests/unit/contexts/run/run.terminal-workflow-service.spec.ts)、[`run.terminal-workflow-concurrency.spec.ts`](../../../tests/unit/contexts/run/run.terminal-workflow-concurrency.spec.ts)、[`run.terminal-workflow-application-shutdown.spec.ts`](../../../tests/unit/contexts/run/run.terminal-workflow-application-shutdown.spec.ts)、[`run.managed-service-launcher.spec.ts`](../../../tests/unit/contexts/run/run.managed-service-launcher.spec.ts)、[`run.service-port-lease-registry.spec.ts`](../../../tests/unit/contexts/run/run.service-port-lease-registry.spec.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Unit / Presentation | 表单解析、快捷执行路由、多运行事件投影、节点运行状态/精确停止入口、通知生命周期、节点定位和连线 | [`run.terminal-workflow-session-events.spec.ts`](../../../tests/unit/contexts/run/run.terminal-workflow-session-events.spec.ts)、[`run.terminal-runtime-header.presentation.spec.tsx`](../../../tests/unit/contexts/run/run.terminal-runtime-header.presentation.spec.tsx)、[`block-graph.terminal-metadata-form.presentation.spec.tsx`](../../../tests/unit/contexts/block-graph/block-graph.terminal-metadata-form.presentation.spec.tsx)、[`block-graph.quick-execution-projection.spec.ts`](../../../tests/unit/contexts/block-graph/block-graph.quick-execution-projection.spec.ts)、[`quick-execution-dispatch.spec.ts`](../../../tests/unit/presentation/quick-execution-dispatch.spec.ts)、[`app-notifications.spec.tsx`](../../../tests/unit/presentation/app-notifications.spec.tsx)、[`terminal-workflow-notifications.spec.ts`](../../../tests/unit/presentation/terminal-workflow-notifications.spec.ts)、[`terminal-workflow.notification-publishing.spec.tsx`](../../../tests/unit/presentation/terminal-workflow.notification-publishing.spec.tsx)、[`terminal-workflow.concurrent-presentation.spec.tsx`](../../../tests/unit/presentation/terminal-workflow.concurrent-presentation.spec.tsx)、[`block-graph.visible-terminal-canvas-target.spec.ts`](../../../tests/unit/contexts/block-graph/block-graph.visible-terminal-canvas-target.spec.ts)、[`terminal-workflow-edges.spec.ts`](../../../tests/unit/presentation/terminal-workflow-edges.spec.ts)、[`terminal-tooltips.spec.tsx`](../../../tests/unit/presentation/terminal-tooltips.spec.tsx) |
...
4. 同一工作区两个节点不相交的长期服务流程可以分别达到并保持就绪；每条活动通知和根终端的“停止本次运行”只停止其绑定的 `runId`，不影响另一条流程。
5. 重启应用后连接和执行配置恢复，旧运行状态不恢复。
6. 活动通知在运行、就绪和终态之间原地更新；关闭活动通知不停止流程，普通更新不重开，后续失败会重新提示。
7. 每个失败工作流按运行 ID 只出现一条可关闭通知；多个运行的通知可以并存，关闭后节点、连线和终端输出仍保留失败证据。
8. 点击工作流通知正文后定位当前失败、运行、就绪节点或流程起点；折叠组合中的节点定位可见组合，停止和关闭入口仍只执行各自动作。
9. 从项目切换到具有同工作区 ID、同终端 ID 的另一项目后，旧通知不得定位或重新发布到新项目；屏幕阅读器仍能获得流程状态与失败摘要或流程范围。
