...
活动运行以项目 ID/目录、工作区名称/目录和 Git 分支作为权威作用域，并以“项目目录 + 工作区 ID + 运行 ID”索引具体 `WorkflowRun`。同一项目的同一工作区可以保留多个运行投影，但任意两个活动运行的终端节点集合必须互不重叠；不相交运行可以分别启动、查询和停止，不需要先放入终端组合。新计划与活动运行共享终端时，Run 在节点 owner 启动队列内原子拒绝新计划，不停止或替换已有运行；新计划只与旧终态投影重叠时，用新运行替换这些节点的旧投影。不同项目即使都使用 `main` 工作区也必须彼此隔离，不能消费对方的运行事件。每个节点 PTY 继续使用完整 `sessionId + runId + generation` 隔离迟到事件。
...
同一项目的同一工作区可以同时保留多个活动工作流，只要它们的计划节点集合互不重叠。启动入口先生成不可变计划，再由 Run 应用服务按计划中的终端 owner 串行化注册：没有节点交集时独立运行；与活动运行存在交集时返回 `TERMINAL_WORKFLOW_SCOPE_CONFLICT`，已有运行保持不变；只与旧终态投影相交时移除旧投影并注册新运行。不同项目或不同工作区继续彼此隔离。
