...
当前 UI 可以从终端节点的“从此处运行流程”、组合的“启动组合命令”或模板库的“放置并运行”发起，不提供空闲态全局运行按钮。普通“启动命令”只运行当前终端，不触发依赖级联。组合启动通过一次工作流请求提交成员作用域，表现层不得逐成员直接启动；同一组合中的独立根和不同流程可以并行，但每条流程仍按直接依赖推进。模板运行必须使用实例化返回的精确作用域，不能重新从模板快照拼装或复制调度规则。
