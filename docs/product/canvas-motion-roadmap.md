...
| 5    | 统一画布指针           | 直接操控保持同一高对比纸飞机指针，不发生状态形变   | 已完成 |

阶段按顺序建立稳定边界。第一阶段不主动改变手感参数；第二阶段的曲线调整必须建立在统一入口上；第三阶段不得绕过 React Flow 的直接操控另建并行 viewport；第四阶段不得让对象装饰影响终端正文或节点几何事实；第五阶段不得以 JavaScript 跟随指针或牺牲文本、连接和 resize 语义换取统一外观。
...
让画布直接操控保持稳定、克制的指针形态，不因平移、选择、拖动或放置状态发生视觉跳换，同时不干扰真正需要语义指针的精确操作。

### 验收边界

- 空白 pane、平移、框选、Terminal、终端组合和 Agent 的选择与拖动、模板放置共享同一无尾纸飞机指针。
- 指针由浏览器原生 CSS cursor 呈现，不建立逐帧跟随的 DOM、React 状态或动画。
- xterm、文本编辑、连接 handle、resize、禁用与等待状态继续使用对应语义指针。
- 指针资源加载失败时回退系统箭头，交互和画布事实不受影响。

### 第五阶段完成证据

第五阶段于 2026-08-04 完成：

- 根级 Presentation 通过单一 `--cc-canvas-pointer` token 引用本地静态矢量纸飞机指针；24px 资源使用深色主体、浅色细描边和左上尖端热点，在明暗画布上保持可辨，并提供系统 `default` 回退。轮廓没有传统箭杆，也不叠加模糊光晕。
- React Flow 的空白、draggable、dragging、selection、selectable 和节点 dragging 状态，以及三类对象标题和框选区域统一消费同一 token；原有 `grab`、`grabbing` 与模板放置 `crosshair` 不再参与核心画布直接操控。
- 统一规则只作用于画布 pane、节点和标题。子元素显式 cursor 继续覆盖继承值，因此 xterm、文本输入、连接 handle、resize、按钮禁用和等待状态保持原语义。
- 实现没有 pointer listener、React 状态或 requestAnimationFrame，不会为了外观引入输入延迟、额外渲染或抖动。
- Unit 验证单一 token、全部核心选择器、模板放置、系统回退、静态矢量资产和语义例外；隔离 Electron E2E 验证画布与真实 Terminal 节点在 idle/dragging 下得到同一浏览器 computed cursor；现有画布、Terminal、Agent 和完整门禁继续通过。
