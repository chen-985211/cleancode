...
| 5    | 统一画布指针           | 直接操控保持同一高对比箭头，不发生状态形变         | 已完成 |
...
- 空白 pane、平移、框选、Terminal、终端组合和 Agent 的选择与拖动、模板放置共享同一箭头。
...
- 根级 Presentation 通过单一 `--cc-canvas-pointer` token 引用本地静态矢量箭头；24px 资源使用深色主体、浅色细描边、轻阴影和尖端热点，在明暗画布上保持可辨，并提供系统 `default` 回退。
