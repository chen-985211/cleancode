...
| 品牌显露 | 延迟 `280ms`，持续 `700ms`                     | 初始空画布的单次字标显露                 |
...
桌面端恢复完成且没有项目时，画布直接呈现无卡片边界的 `CleanCode` 品牌和打开项目主操作，不叠加欢迎面板、说明或装饰图标。打开项目使用中性浅色表面、细边框和前景色，不使用主色实心填充。品牌只在该空状态首次挂载时播放一次从紧缩字距到最终字距的透明度显露；挂载后先保留一段短暂视线落位时间，再以明显可辨但不拖沓的节奏展开，操作始终稳定可用。加载、恢复失败和浏览器预览继续使用各自的状态表面，不复用品牌演出。`prefers-reduced-motion` 下取消等待并直接显示最终字距。
