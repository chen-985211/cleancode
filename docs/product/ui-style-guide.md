...
组合的展开与收起使用相同空间关系：成员从折叠组合中心回到持久化位置，并在视觉 `.terminal-node` 上从轻微缩小和淡化收敛到完整状态；收起沿镜像顺序和反向路径返回。成员级联必须确定且总延迟不超过 `60ms`，不得随成员数量线性拖长。React Flow 组合根先提交最终几何，同一层 `.terminal-group-node__material` 再从上一世界矩形连续变形到当前矩形；不得同时叠放新旧尺寸外壳，不得非等比缩放真实组合根、圆角、阴影、handle、resize 命中区或内部内容。展开与收起使用快速、无回弹的临界阻尼 spring，收起可以略快于展开；加入、离开和组内重排同样从当前屏幕位置收敛。快速反向必须从当前外壳世界矩形和成员 presentation 重新定向，取消尚未开始的级联等待，并只继承朝向新目标的速度，不能先回到端点、排队或重新播放。`prefers-reduced-motion` 下取消位移、级联和尺度变化，直接投影同一最终状态。
