...
MCP 终端搭建的空间起点优先使用发起 Agent 的边缘，只有发起 Agent 不在当前投影中时才回退到最终落位附近最近的既有画布节点。相机先以发起 Agent 与本次已提交结果的最终边界完成一次可中断聚焦，节点随后从同一因果起点开始搭建；聚焦与搭建不得并行制造两套运动，也不得在逐步显露期间重复追踪。观看范围应保留来源与结果的关系，同时维持终端可读性；距离异常时由统一相机 owner 的缩放边界约束结果，不在局部动效中另建镜头算法。
