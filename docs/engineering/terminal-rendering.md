...
架构规则以 [架构文档](architecture.md) 为准，测试层级与门禁以 [测试规范](../testing/testing.md) 为准，Electron、xterm.js 和 node-pty 的技术选择以 [技术栈说明](tech-stack.md) 为准，Agent 控制台和终端积木的稳定用户界面契约以 [UI 契约](../product/ui.md) 为准。本文记录的具体选择属于当前技术栈下的实现与排障方式；若依赖版本或渲染器变化，必须重新测量，不得把本文中的一次观测值当成永久常量。
