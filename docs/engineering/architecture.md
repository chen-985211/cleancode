...
当前持久化实现位于 Electron 应用数据目录：项目及当前项目选择、积木图、工作区 Agent 定义及其会话绑定使用版本化 JSON，Agent 审计记录使用 JSONL。工作区 Agent 存储支持从旧版单 Agent 分支绑定迁移，并必须保留原有 Codex thread UUID。写入持久化业务状态必须通过应用层仓储端口；需要替换为 SQLite 时，只能替换基础设施实现，不得改变领域和应用层契约。
