...
- `WorkspaceInitialization`：项目上下文的一次工作区内容初始化聚合根，拥有逐项创建进度和启动请求凭据。
...
当前持久化实现位于 Electron 应用数据目录的当前状态代际：项目及当前项目选择、积木图、工作区 Agent 定义及其会话绑定使用版本化 JSON，Agent 审计记录使用 JSONL。积木图仓储写入版本 `5` 并兼容读取版本 `2`、`3`、`4` 与 `5`，工作区 Agent 仓储写入 schema v6 并兼容读取 v5；不受支持或畸形的旧版本不迁移、不回写，项目内 `.cleancode` 状态同样不会被读取、迁移或回写。产品尚未公开期间通过新的状态根生成全新当前数据。Run 终端恢复目录另存 checkpoint 和有界追加输出；它只用于技术恢复，不得被 Project、BlockGraph、UI 或 Agent 当作可编辑业务仓储。写入持久化业务状态必须通过应用层仓储端口。

Project 另以独立 schema v1 保存工作区默认内容与逐项初始化进度，通过应用层端口使用 BlockGraph 的冻结模板及实例化、Agent 的显式创建和 Run 的既有启动入口。创建凭据留在产生对象的上下文，Project 不复制运行时状态；初始化中断恢复不等于恢复工作流运行实例。
