...
- 运行上下文：负责普通终端 PTY 会话、终端工作流计划、运行状态、就绪探测、停止和失败传播。
...
- `ProjectRegistry`：项目上下文的最近项目目录登记簿聚合根。
...
- 最近项目目录列表的唯一事实来源：`ProjectRegistry` 聚合。
...
基础设施层负责执行外部命令、启动 PTY、探测服务就绪和发布运行事件。当前活动运行只存在于进程内存中，不持久化运行实例。
