...
Windows 原生 TUI 可以直接通过 Console handle 发出 OSC 10/11，并在很短的等待后回退读取 screen buffer 属性。旧版系统 ConPTY 会在 `node-pty.onData` 之前消费查询和鼠标模式；Run 固定使用 node-pty 随包的 ConPTY DLL，在 renderer 或 headless model 回答初始终端能力握手后透传完整 VT 序列，使查询继续遵守既有 attach/detach 响应权交接，并使 SGR mouse 模式到达可见 xterm。Codex 0.145.0 的 Windows 路径会在 100 ms 查询窗口后调用 `GetConsoleScreenBufferInfoEx`，所以 Windows Agent foreground 脚本仍必须在内部 started 控制帧之前按固定源主题设置 ConsoleColor，浅色为 `Black`/`White`、深色为 `Gray`/`Black`，使同步 fallback 已经得到正确明暗；这些索引由 renderer xterm 的 canonical ANSI palette 映射，准备阶段输出由 transport 过滤。不得向尚未查询的进程输入预置 OSC 响应，否则无查询的 CLI 或退出后的 shell 会把它们当作用户输入。
