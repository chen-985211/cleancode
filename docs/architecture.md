...
- `BlockGraph` 是终端执行配置与终端间有向连接的事实来源，负责拒绝自连接、重复连接和环，并在删除积木时清理关联连接。
- `Run` 边界中的 `WorkflowRun` 持有启动时生成的不可变计划，负责等待、运行、就绪、成功、失败、阻塞和停止状态，以及并行、汇合、失败传播和逆拓扑停止。
- 应用层通过稳定端口从 `BlockGraph` 获取计划，禁止 `Run` 直接读取或修改 `BlockGraph` 聚合内部状态。
- 任务终端通过独立命令 PTY 的真实进程退出码判断成功；结束后可以切换为空交互式 shell，并保留已经产生的输出。
- 服务终端通过字面量输出或本机 TCP 端口判断就绪；服务就绪后保持运行，意外退出视为失败。
- 工作流不解析 shell 提示符，不把标准输出自动传给下游，也不把人工键盘输入当作依赖完成信号。
- 上游失败只阻塞其后代，互不依赖的分支继续运行；停止时按逆拓扑顺序终止已启动节点。
- 当前活动工作流只保存在进程内存中；工作区持久化的是终端执行配置和连接，不持久化运行实例。
...
当前持久化实现位于 Electron 应用数据目录：项目、积木图、工作区 Agent 定义及其会话绑定使用版本化 JSON，Agent 审计记录使用 JSONL。工作区 Agent 存储支持从旧版单 Agent 分支绑定迁移，并必须保留原有 Codex thread UUID。写入持久化业务状态必须通过应用层仓储端口；需要替换为 SQLite 时，只能替换基础设施实现，不得改变领域和应用层契约。
