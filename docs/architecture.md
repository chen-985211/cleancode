...
- Agent 对话恢复绑定的唯一事实来源：`AgentSession` 聚合及其仓储。绑定键由项目、工作区和 Git 分支组成，绑定值是 Codex thread UUID。
...
Agent 会话必须按项目、工作区和 Git 分支隔离。同一物理工作目录在同一时刻最多允许一个分支作用域的可写 Agent PTY；项目上下文切换该目录的 Git 分支前，必须通过应用层端口等待旧 Agent PTY 挂起。Git 检出失败时必须恢复旧作用域，检出成功后由新分支作用域接管。非 Git 项目使用显式的无分支持久化作用域；Git detached HEAD 没有稳定分支身份，其 Agent 会话必须是易失的，不得复用无分支绑定。
...
当前持久化实现位于 Electron 应用数据目录：项目、积木图和 Agent 会话绑定使用版本化 JSON，Agent 审计记录使用 JSONL。写入持久化业务状态必须通过应用层仓储端口；需要替换为 SQLite 时，只能替换基础设施实现，不得改变领域和应用层契约。
