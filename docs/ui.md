...
从“快速启动”进入配置时，编辑表单必须直接聚焦启动命令输入框。

### 快速启动

快速启动入口必须直接放在终端积木头部的主要操作区，而不是藏在编辑表单或二级菜单里。

快速启动按钮必须用可见状态区分是否已经配置启动命令：
...
当启动命令为空时，点击快速启动必须打开终端信息编辑表单，并聚焦启动命令输入框。

当启动命令已配置时，点击快速启动必须按以下语义执行：
...
快速启动不得把启动命令喂给正在运行的旧项目进程。这样可以避免旧进程吞掉命令、输出混在一起或同一个启动命令被运行多次。

快速启动过程中，同一个终端积木必须防抖或互斥，避免用户连点造成重复启动。

快速启动入口仍只属于单个终端积木。组合终端的“启动全部”会对每个成员终端执行批量启动：已配置启动命令的成员必须按快速启动语义替换旧会话、清空可见输出并写入启动命令一次；未配置启动命令的成员只启动新的终端会话。
...
进入组合选择模式后，画布工具栏必须显示选择提示和已选择数量。终端积木必须显示可选状态；已在其他组合内的终端不得被选入新组合。

创建组合至少需要两个终端。少于两个可用终端时，“组合终端”入口必须禁用。

折叠后的终端组合必须展示成员摘要列表。成员摘要必须包含成员名称、状态提示和移出入口。

当组合成员少于两个时，系统必须阻止继续形成无意义组合，或自动解散组合。UI 不得展示只有一个成员却仍可作为有效组合操作的状态。
...
- 快速启动入口位于终端头部；未配置时进入配置，已配置时重启该终端会话、清空可见输出并只执行一次启动命令。
- 用户可以把多个终端创建为终端组合，并对组合执行启动全部、停止全部、重启全部、折叠、展开、添加成员、移出成员和解散。
