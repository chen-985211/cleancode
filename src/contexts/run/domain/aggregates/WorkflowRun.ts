...
export class WorkflowRun {
  ...
  static create(plan: WorkflowRunPlanSnapshot, id = createRunId()): WorkflowRun {
    return new WorkflowRun(id, plan)
  }
  ...
  markFailed(blockId: string, reason: string): void {
    const node = this.requireNode(blockId)
  ...
  toSnapshot(): WorkflowRunSnapshot {
    return {
      ...
      workspaceName: this.plan.workspaceName,
  ...
  private resolveStatus(): WorkflowRunStatus {
    ...
    if (this.nodes.some((node) => node.status === 'failed')) {
      return 'failed'
    }
    if (this.nodes.some((node) => node.status === 'ready')) {
      return 'ready'
    }
