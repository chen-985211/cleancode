...
} from '../terminal-surface/terminalPaste'
import { bindTerminalSurfaceAttachmentIdentity } from '../terminal-surface/terminalSurfaceAttachmentIdentity'
import {
  createTerminalSurfaceKey,
  type TerminalSearchResults,
  type TerminalSurface
} from '../terminal-surface/terminalSurfaceRegistry'
import { readTerminalSourceTheme } from '../terminal-surface/terminalTheme'
import {
  attachTerminalViewWithRetry,
  restoreTerminalViewWithRetry
} from '../terminal-surface/terminalViewAttachment'
import { createTerminalXtermSurface } from '../terminal-surface/terminalXtermSurface'
import { useTerminalSurfaceRegistry } from '../terminal-surface/useTerminalSurfaceRegistry'
import { appendTerminalOutputTail } from '../view-models/terminalOutputTail'
import type {
  TerminalDimensions,
  TerminalViewState
} from '../view-models/TerminalPresentationTypes'
import { useI18n } from '../../../../presentation/i18n/useI18n'

export interface TerminalRuntimeViewportProps {
...
export function TerminalRuntimeViewport({
  blockName,
  session,
  focusRequestId,
  ...
  useEffect(() => {
    const controller = new TerminalPasteController({
      write: (chunk) =>
        isInputDisabledRef.current ? Promise.resolve() : onPasteRef.current(chunk),
      onStateChange: (state) => {
        setPasteState(state)
  ...
  useEffect(() => {
    ...
    surface.attach({
      ...
      onInput: (input) => {
        if (!isInputDisabledRef.current) onInputRef.current(input)
      },
  ...
  if (isTestRuntime()) {
    return (
      <pre
        ...
        aria-label={t('terminal.output', { blockName })}
